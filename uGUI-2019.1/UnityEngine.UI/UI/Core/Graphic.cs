...
namespace UnityEngine.UI
{
    ...
    [RequireComponent(typeof(RectTransform))]//依赖于CanvasRenderer画布渲染组件和RectTransform组件
    ...
    /// </code>
    /// </example>
    public abstract class Graphic//抽象类 不能被实例恶化
        : UIBehaviour,
          ICanvasElement
    {
        ...
        static public Material defaultGraphicMaterial//默认材质：当未指定材质是使用默认材质
        {
            get
            {
                if (s_DefaultUI == null)
        ...
        /// </summary>
        public virtual void SetAllDirty()//将元素添加到布局和图像重建中
        {
        ...
        private void CacheCanvas()//找到渲染当前元素的Canvas
        {
        ...
        protected override void OnEnable()
        {
            ...
            SetAllDirty();//将元素添加到布局和图像重建中(这样说有些不严谨，因为布局重建是通过LayoutRebuilder类管理的，如果父物体身上没有继承ILayoutGroup的组件是不会添加到布局重建序列中的)
        }
...
#endif
            GraphicRegistry.UnregisterGraphicForCanvas(canvas, this);
            CanvasUpdateRegistry.UnRegisterCanvasElementForRebuild(this);//把当前元素从GraphicRegistry与CanvasUpdateRegistry中清除

            if (canvasRenderer != null)
                canvasRenderer.Clear();//清理canvasRenderer

            LayoutRebuilder.MarkLayoutForRebuild(rectTransform); //将元素添加到布局重建序列中(因为Graphic已经Disable所以不需要将元素添加到图像重建序列中)
