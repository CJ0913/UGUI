...
namespace UnityEngine.UI.Collections
{
    internal class IndexedSet<T> : IList<T>
    {
        ...
        {
            m_List.Add(item);
            m_Dictionary.Add(item, m_List.Count - 1);//key:存储的数据  value:数据在List中的下标
        }
        ...
        public void RemoveAt(int index)
        {
            ...
            if (index == m_List.Count - 1)//如果是在最后面就直接删除
                m_List.RemoveAt(index);
            else//如果不是在最后面，把要删除的元素与最后的元素交换位置，再删除最后的位置
            {
        ...
        {
            get { return m_List[index]; }//获取
            set//赋值：先把字典里对应的元素的删除，再将将要赋值的元素加入字典
            {
                T item = m_List[index];
                m_Dictionary.Remove(item);
                m_List[index] = value;
